...
Predicts time to resolution using quantile regression
to produce confidence intervals (P25/P50/P75 days).
"""
...
from ..config import DurationModelConfig, MODEL_VERSION


class DurationModel(BaseModel):
    """
    Model to predict case duration (days to resolution).

    Uses GradientBoostingRegressor with quantile loss for
    multi-quantile prediction, providing confidence intervals.
    """

    def __init__(self, config: Optional[DurationModelConfig] = None):
        """
    ...
    def fit(
        ...
        **kwargs
        ...
        """
        Train quantile regression models for duration.
        ...
        """
        # Clip training targets
        y_clipped = self._clip_predictions(y)
        ...
        self.metadata = self._create_metadata(
            ...
            metrics={
                ...
                'median_duration': float(np.median(y)),
            },
            description="Case duration quantile regression model"
        )
    ...
    def predict(self, X: np.ndarray) -> np.ndarray:
        """
        Predict median duration.

        Args:
            X: Feature matrix

        Returns:
            Array of predicted median durations (days)
        """
    ...
    def predict_quantiles(self, X: np.ndarray) -> Dict[float, np.ndarray]:
        ...
        Returns:
            Dict of quantile: predictions (days)
        """
        if not self.is_fitted():
            raise ValueError("Model must be fitted before prediction")
        ...
        for quantile, model in self.quantile_models.items():
            ...
            results[quantile] = self._clip_predictions(predictions)

        return results

    def predict_range(self, X: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        """
        Predict low/mid/high durations.

        Args:
            X: Feature matrix

        Returns:
            Tuple of (low, mid, high) arrays (P25, P50, P75 days)
        """
    ...
    def predict_with_confidence(
        ...
        for i in range(len(mid)):
            ...
            if mid[i] > 0:
                cv = range_width / mid[i]
                confidence = max(0, 1 - min(cv, 1))
            else:
                confidence = 0.5
    ...
    def load(self, path: Path) -> 'DurationModel':
        ...
        self.config = model_data.get('config', DurationModelConfig())
        self.model = self.quantile_models.get(0.5)
